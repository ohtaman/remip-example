...
あなたは、大規模なEC（電子商取引）プラットフォームの物流部門の配車担当者です。複数の物流拠点（デポ）から、多数の顧客へ商品を配送する日々の計画を立てる必要があります。

保有するトラックはサイズや種類が異なり、それぞれ積載量やコストが異なります。また、ドライバーの労働時間には上限があり、顧客からは配送の時間枠（例: 午前中、14-16時など）が指定されています。
...
| パラメータ名 | 値 | 単位 |
| :--- | :--- | :--- |
| `driver_max_work_hours` | 8.0 | 時間 |
| `average_speed_km_per_hour` | 40.0 | km/h |
| `wait_penalty_per_hour` | 1000 | 円/時間 |
| `delay_penalty_per_hour` | 3000 | 円/時間 |
| `external_sourcing_base_cost` | 5000 | 円/件 |
| `external_sourcing_variable_cost` | 100 | 円/kg |

### 5.2. 車両マスタ (`vehicles`)

| vehicle_id | depot_id | type | capacity_kg | cost_per_km |
| :--- | :--- | :--- | ---:| ---:|
...
| depot_id | location_x | location_y |
| :--- | ---:| ---:|
| D1 | 50 | 50 |
| D2 | 5 | 80 |
...
| customer_id | location_x | location_y | demand_kg | time_window_start | time_window_end | service_time_min |
| :--- | ---:| ---:| ---:| ---:| ---:| ---:|
...
### 5.5. 拠点-顧客間の距離マスタ (`distance_matrix`)

- `distance_km`: 2点間のユークリッド距離
- 移動時間は `distance_km` / `average_speed_km_per_hour` で計算可能
